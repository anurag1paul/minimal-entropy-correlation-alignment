...
class logDcoral(object):
    ...
    def __init__(self, mode='training', method='baseline', hidden_size=128,
                 learning_rate=0.0001, batch_size=256, alpha=1.0):
        self.mode = mode
        self.method = method
        ...
        with tf.variable_scope('encoder', reuse=reuse):
            with slim.arg_scope([slim.fully_connected],
                ...
                with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.relu,
                    ...
                    net = slim.fully_connected(net, self.hidden_repr_size,
    ...
    def logits(self, inputs, is_training=False, reuse=False):
        ...
        with tf.variable_scope('logits', reuse=reuse):
            with slim.arg_scope([slim.fully_connected], activation_fn=None):
                return slim.fully_connected(inputs, 10, activation_fn=None,
                                            scope='fc5')
    ...
    def build_model(self):
        ...
        if self.mode == 'train':
            self.src_images = tf.placeholder(tf.float32, [None, 32, 32, 3],
                                             'svhn_images')
            self.trg_images = tf.placeholder(tf.float32, [None, 32, 32, 1],
                                             'mnist_images')
            self.src_labels = tf.placeholder(tf.int64, [None], 'svhn_labels')

            self.src_hidden = self.E(self.src_images, is_training=True)
            self.trg_hidden = self.E(self.trg_images, is_training=True,
                                     reuse=True)
            ...
            self.src_accuracy = tf.reduce_mean(
                tf.cast(self.src_correct_pred, tf.float32))

            # losses: class, domain, total
            self.class_loss = slim.losses.sparse_softmax_cross_entropy(
                self.src_logits, self.src_labels)

            self.trg_softmax = slim.softmax(self.trg_logits)
            self.trg_entropy = -tf.reduce_mean(
                tf.reduce_sum(self.trg_softmax * tf.log(self.trg_softmax),
                              axis=1))
            ...
            if self.method == 'log-d-coral':
                ...
                print('----------------')
                self.domain_loss = self.alpha * self.log_coral_loss(
                    self.src_hidden, self.trg_hidden)
                ...
                print('----------------')
                self.domain_loss = self.alpha * self.coral_loss(self.src_hidden,
                                                                self.trg_hidden)
                self.loss = self.class_loss + self.domain_loss
            ...
            class_loss_summary = tf.summary.scalar('classification_loss',
                                                   self.class_loss)
            domain_loss_summary = tf.summary.scalar('domain_loss',
                                                    self.domain_loss)
            src_accuracy_summary = tf.summary.scalar('src_accuracy',
                                                     self.src_accuracy)
            trg_entropy_summary = tf.summary.scalar('trg_entropy',
                                                    self.trg_entropy)
            ...
            self.trg_accuracy = tf.reduce_mean(
                tf.cast(self.trg_correct_pred, tf.float32))
            ...
            self.trg_hidden = self.E(self.trg_images, is_training=False)
            self.src_hidden = self.E(self.src_images, is_training=False,
                                     reuse=True)
